...
export async function PUT(request: NextRequest) {
  ...
  if (!id || !document) {
    return NextResponse.json({ message: 'Missing id or document parameter' }, { status: 400 });
  }


  // Prepare the API call to the backend
  const url = `${getBaseUrl()}/Patient/updatePatientDocument`;
  ...
  try {
    ...
    return NextResponse.json({ message: err.message || 'Failed to update patient document' }, { status: 500 });
  }
}